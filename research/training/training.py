...
************************************
  ...
  # Define the loss:
  research.losses.log_loss(predictions, labels)
  total_loss = research.losses.losses.get_total_loss()
  ...
  # Create the train_op
  train_op = research.training.create_train_op(total_loss, optimizer)

  # Run training.
  research.training.train(train_op, my_log_dir)
...
(c) returns the value of the loss. research.training.create_train_op creates
...
the gradients using a few arguments:
  ...
  train_op = research.training.create_train_op(
      total_loss,
      optimizer,
  ...
  def transform_grads_fn(grads):
    ...
    return research.training.multiply_gradients(
...
of non-gradient updates during training  research.training.create_train_op
...
By default, research.training.create_train_op includes all update ops that are
part of the `tf.GraphKeys.UPDATE_OPS` collection. Additionally, the
research.batch_norm function adds the moving mean and moving variance
updates to this collection. Consequently, users who want to use
research.batch_norm will not need to take any additional steps in order
...
One can use a tf.Scaffold and an initializing function to do so.
  ...
  variables_to_restore = research.get_model_variables()
  init_fn = tf.assign_from_checkpoint_fn(
      checkpoint_path, variables_to_restore)

  # Run training.
  scaffold = tf.Scaffold(init_fn=init_fn)
  research.training.train(train_op, my_log_dir, scaffold=scaffold)
...
above:
  ...
  variables_to_restore = {
      'name_var_0_in_checkpoint':
          research.get_unique_variable('var0'),
      'name_var_1_in_checkpoint':
          research.get_unique_variable('var1')
  }
  init_fn = research.framework.assign_from_checkpoint_fn(
        checkpoint_path, variables_to_restore)
  scaffold = tf.Scaffold(init_fn=init_fn)
...
need only filter those variables to initialize as follows:
  ...
  # patterns:
  variables_to_restore = research.get_variables_to_restore(
      include=["conv"], exclude=["fc8", "fc9])
  # or
  variables_to_restore = research.get_variables_to_restore(
      exclude=["conv"])

  init_fn = research.assign_from_checkpoint_fn(
      checkpoint_path, variables_to_restore)
  scaffold = tf.Scaffold(init_fn=init_fn)
...
placeholders and a feed dictionary:
  ...
  init_fn = research.assign_from_values_fn(var_names_to_values)
  scaffold = tf.Scaffold(init_fn=init_fn)
