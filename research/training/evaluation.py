...
out the metrics values to stdout:
  ...
  names_to_values, names_to_updates = research.metrics.aggregate_metric_map({
  ...
  names_to_values = evaluate_once(
      ...
      hooks=[
            research.evaluation.StopAfterNEvalsHook(num_evals),
            research.evaluation.SummaryAtEndHook(logdir),
      ],
      config=None)
...
more summaries and call the evaluate_repeatedly method:
  ...
  research.evaluation.evaluate_repeatedly(
      checkpoint_dir,
      eval_ops=names_to_updates.values(),
      hooks=[
            research.evaluation.StopAfterNEvalsHook(num_evals),
...
def evaluate_repeatedly(checkpoint_dir,
  ...
  `research.train_eval.StopAfterNEvalsHook` which results in `eval_ops` running
  ...
  evaluations run via `research.evaluation.get_or_create_eval_step`.
  ...
  Args:
    ...
    eval_ops: A single `Tensor`, a list of `Tensors` or a dictionary of names to
      ...
      done by a `research.evaluation.StopAfterNEvalsHook`.
